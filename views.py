...
stripe.api_key = 'get_random_secret_key()'
...
#    Room = room.objects.filter(Premium = True,Active = True)
    ...
    try:
        data = respo.json()
        room_details = data.get('results', [])
    except Exception as e:
        print("⚠️ Failed to parse JSON response:", respo.text)
        print("Error:", e)
        room_details = []

    return render(request, 'index11.html', {
        'RoomDetail': room_details,
        'state': state,
        'gallery': gallery
    })
...
def loginpage(request):
    Render_to = str(request.GET.get('next', '/'))
    print(Render_to)
    ...
    def create(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            send_mail("VALIDATE POST", " Validate This Post  to upload : http://127.0.0.1:8000/rent/validationpage/",
                       'help.CBUHomeRentals@gmail.com', ['help.CBUHomeRentals@gmail.com'])
...
def register(request):
    if request.method == 'POST':
        username = request.POST.get('your_email')
        ...
        if user is not None:
            ...
            return redirect('/rent/loginPage/')
